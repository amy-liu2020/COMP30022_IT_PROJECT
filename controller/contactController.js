        ...
        return Promise.all(
            mids.map(async (mid) => {
                ...
                if (!meeting) {
                    ...
                    thisMeeting.Invitees.splice(0, 0, {
...
const assignTag = async (req, res) => {
    ...
    let tag = await Tag.findOne(
        { TagName: tagName, TagOf: "C", AccountID: uid },
        "TagName",
        (err) => {
            if (err) {
                res.status(400).json({
    ...
    Contact.findByIdAndUpdate(
        cid,
        { $push: { Tags: { TagName: tag.TagName, TagId: tag._id } } },
        (err) => {
            if (err) {
                res.status(400).json({
                    msg: "Error occurred: " + err,

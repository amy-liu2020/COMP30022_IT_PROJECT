...
const getProfile = async (req, res) => {
    try {
        let uid = req.token.userId;
        var thisAccount = await User.findOne(
            { UserID: uid },
            "Email PhoneNumber UserName UserID",
            (err) => {
                if (err) {
                    res.status(400).json({
                        msg: "Error occurred: " + err,
...
const forgetPassword = async (req, res) => {
    try {
        ...
        let question = await SecurityQuestion.findOne(
            { Code: sqCode },
            (err) => {
                if (err) {
                    res.status(400).json({
...
const changeForgottenPassword = async (req, res) => {
    try {
        ...
        if (Answer !== thisAccount.SecurityAnswer) {
            res.status(403).json({
...
const changePassword = async (req, res) => {
    try {
        ...
        let uid = req.token.userId;
        console.log(encrypt(OldPassword));

        let thisAccount = await User.findOne(
            { UserID: uid, Password: encrypt(OldPassword) },
            (err) => {
                if (err) {
                    res.status(400).json({
                        msg: "Error occurred: " + err,
